...
export default function Cadastro() {
    ...
    const [bolos, setBolos] = useState<Bolos[]>([]);
    ...
    const [imagem, setImage] = useState<File | undefined>(undefined);
    ...
    const abrirModalParaComfimarDelete = (id: string) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => { //ao se comunicar com a api voce precisa fazer funções assincronas (async)
        e.preventDefault(); //preventDefault: ignora o comportamento padrao do html, ele ignora a atualização da página
        if (!nomeBolo || !categorias || !preco) { //se eu não preecher um desses campos vai acontecer:
        ...
        if (imagem) {
            ...
            if (!uploadedFileName) {
                ...
                return; //encerra o fluxo da função 
            }
        }

        const novoBolo: Bolos = {
            id: undefined,
            nome: nomeBolo,
            descricao: descricao,
            preco: preco,
    ...
    return (
        <>
                        ...
                        <div className="cadastro_coluna1">
                            <div className="bolos">
                                ...
                                <input
                                    ...
                                    value={nomeBolo}
                                    onChange={b => setNomeBolo(b.target.value)}
                                />
                            </div>
                            ...
                            <div className="valor_peso">
                                <div className="valor">
                                    ...
                                    <NumericFormat
                                        ...
                                        prefix='R$ '
                                        decimalScale={2} //quantos numeros mostrar nos valores decimais
                                        fixedDecimalScale //para fixar o 2 nas casas decimais(se o usuario colocar outra coisa nao vai funcionar)
                                        allowNegative={false} //permitir numeros negativos false(nao)
                                        onValueChange={(values) => { setPreco(values.floatValue ?? undefined) }} //pega os valores conforme forem digitados
                                        inputMode='decimal'
                                    />
                                </div>
                                ...
                                <div className="peso">
                                    ...
                                    <NumericFormat
                                        id='peso'
                                        placeholder='Inserir peso do produto'
                                        value={peso ?? ""}
                                        thousandSeparator="."
                                        decimalSeparator=','
                                        decimalScale={3}
                                        ...
                                        suffix=' kg'
                                        inputMode='decimal'
                                        onValueChange={(values) => { setPeso(values.floatValue ?? undefined) }}
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <tbody>
                            {
                                bolos.map((b: Bolos) => (
                                    <tr>
                                        ...
                                        <td data-cell="Categoria: ">{b.categorias.map(c => c.charAt(0).toUpperCase() + c.slice(1)).join(", ")}</td>  {/* Join: serva para você adicionar algum tipo de espaçamento, por exemplo uma virgula */}
                                        ...
                                        <td>
                                            <svg onClick={() => abrirModalParaComfimarDelete(b.id!)} xmlns="http://www.w3.org/2000/svg"
                                                viewBox="0 0 640 640">
                                                <path fill="currentColor"
            ...
            <ModalCustomizado
                ...
                titulo='Sucesso'
                corpo='Bolo removido!' exibirConteudoCentralizado={false}
            />
            <ModalCustomizado
